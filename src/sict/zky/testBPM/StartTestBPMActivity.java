...
import sict.zky.deskclock.ToastMaster;
...
public class StartTestBPMActivity extends Activity {
	...
	private final static String TAG = StartTestBPMActivity.class
			.getSimpleName();
	...
	// private int count = -1;
	private int[] historyGLUValue = new int[20];
	...
	private Spinner starttestbpm_spinner;
...
//			Toast.makeText(getApplicationContext(), checkString, 1).show();
			// 如果是信息包，发送05应答包
...
//				Toast.makeText(getApplicationContext(), "收到信息包", 0).show();
				Toast.makeText(getApplicationContext(), "如无法建立连接，请返回到主界面，再进入测试界面，并重新打开血压仪", 1).show();
				SampleGattAttributes.sendMessage(mBluetoothGatt, replyPacket);
				
//				Toast.makeText(getApplicationContext(), "应答包："+replyPacket, 0).show();
			}
...
//				Toast.makeText(getApplicationContext(), "收到开始包", 0).show();
//				Toast.makeText(getApplicationContext(), "应答包："+replyPacket, 0).show();
			}
			...
			if (resultofMsg.length() >= 16) {
				...
				if (pk.isProcessPacket(resultofMsg)) {
					...
					String pre = Integer
							.toString(Integer.parseInt(Presure, 16));
...
//			else {
//				Toast.makeText(getApplicationContext(), resultofMsg, 1).show();
//			}
			...
			if (resultString.length() > 6) {
				if (pk.isBPMResultPacket(resultString)) {
					...
					SampleGattAttributes.sendMessage(mBluetoothGatt,
							replyPacket);
					...
					String timeValue = year + "-" + month + "-" + day + " "
					...
					if (thisTest) {
						...
						int level = analyresulut.analyse_bpm_result(
								Integer.valueOf(thishigh),
								Integer.valueOf(thislow));
						result(level, thishigh, thislow);

						// 保存本次测量结果值
						// userId需要修改为int
						pc_data = new Pc_data(userId, Integer.parseInt(high),
								Integer.parseInt(low), Integer.parseInt(pul),
								timeValue, userName, screenName_, familyMember_);
						pc_dataService.insert(pc_data);

						thisTest = false;
						if (userId > 0) {
							AlertDialog.Builder dialog = new AlertDialog.Builder(
									StartTestBPMActivity.this);
						...
						historyPULSEValue[countBPMResult] = Integer
								.valueOf(pul);// 每次把值写入historyPULSEValue数组
			...
			// GLU = Integer.toString(Integer.parseInt(str, 16));
			//
			// String testTime = resultString.substring(6, 16);
			//
			// String year = Integer.toString(Integer.parseInt(
			...
			// thisTest = false;
			//
			// pc_bgdata = new Pc_bgdata(userId, Integer.valueOf(ThisGLU),
			// timeValue, userName, screenName_, familyMember_);
			//
			// // pc_bgdata= new Pc_bgdata(userId
			// // ,Integer.parseInt(GLU),timeValue, "me", screenName_ , 1);
			...
			// } else
			// {
			// historyGLUValue[countBPMResult] = Integer.valueOf(GLU);//
			// 每次把值写入historyGLUValue数组
			//
			// historyGLUTime[countBPMResult] = timeValue; //
			// 每次把值写入historyGLUTime数组
			//
			// // 历史数据保存到本地，uername和 familymember 是固定的。。。。
			// pc_bgdata = new Pc_bgdata(userId,
		...
		private void result(int level, String thishigh, String thislow) {
			switch (level) {
			case 1:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_1));
				SYSView.setTextColor(0xFF06ADED);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFF06ADED);
				DIAView.setText("低压   " + thislow);
				break;

			case 2:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_2));
				SYSView.setTextColor(0xFF81C53F);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFF81C53F);
				DIAView.setText("低压   " + thislow);
				break;
			...
			case 4:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_3));
				SYSView.setTextColor(0xFFFFDD52);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFFFFDD52);
				DIAView.setText("低压   " + thislow);
				break;
			...
			case 5:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_4));
				SYSView.setTextColor(0xFFF3980F);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFFF3980F);
				DIAView.setText("低压   " + thislow);
				break;
			...
			case 6:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_5));
				SYSView.setTextColor(0xFFF55273);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFFF55273);
				DIAView.setText("低压   " + thislow);
				break;
			...
			case 7:
				level_image.setImageDrawable(getResources().getDrawable(
						R.drawable.level_6));
				SYSView.setTextColor(0xFFD5143B);
				SYSView.setText("高压   " + thishigh);
				DIAView.setTextColor(0xFFD5143B);
				DIAView.setText("低压   " + thislow);
				break;
	...
	private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			// 如果蓝牙服务没有连接，更新连接状态为断开，并且刷新
				...
				if(thishigh.equals("")){
					Toast.makeText(getApplicationContext(), "与蓝牙的连接已断开，2秒后返回主界面，如需测量，重新进入测试界面。", 1).show();
					new Handler().postDelayed(new Runnable(){  
				        public void run() {  
				        //在这里执行延时任务  
				        	Intent intent1 = new Intent(StartTestBPMActivity.this,
									MainActivity.class);
							intent1.putExtra("userId", userId);
							intent1.putExtra("screenName", screenName_);
...
//				scanLeDevice(true);
			...
			// 如果蓝牙服务数据可用，接受设备广播发出的数据，进行处理
				...
				if (notify != null) {
					...
					// Toast.makeText(getApplicationContext(),
					// notify.toString(), 1).show();
					// SampleGattAttributes.sendMessage(mBluetoothGatt,
					// replyPacket);//发送05应答包？？？ 每次都回应？
					// count++;
					// 主机被动接收的数据
					...
					if (haveinformation && checkString1.length() > 16) {
						resultofMsg = checkString1.substring(0, 16);// 过程包
						checkString1 = checkString1.substring(16);
					}
...
//						SampleGattAttributes.sendMessage(mBluetoothGatt, replyPacket);
					...
					if (builder1.toString().length() == 30) {
						resultString = builder1.toString();
					} else {
						resultString = "Itisnotres!";
					}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.starttestbpmactivity);
...
thishigh="";
		...
		familyMember_ = 0;
		connecting=true;
		// Toast.makeText(getApplicationContext(), userId+" , "+screenName_,
		// 1).show();
		// thisposition = intent.getIntExtra("thisposition", 0);

		starttestbpm_spinner = (Spinner) findViewById(R.id.starttestbpm_spinner);
		...
		starttestbpm_spinner.setAdapter(userNameAdapter);
		// starttestbpm_spinner.setSelection(thisposition, true);
		// starttestbpm_spinner.
		starttestbpm_spinner.setOnItemSelectedListener(new OnItemSelected());

		// userName = (String) starttestbpm_spinner
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (!mDeviceAddress.equals("")
							&& device.getName().indexOf("Bioland-BPM") >= 0)
...
//	}
	...
	Handler bphistoryhandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case 0:
				// mDialog.cancel();
				Toast.makeText(StartTestBPMActivity.this, "上传成功！",
						Toast.LENGTH_SHORT).show();
	...
	class Update_history_pc_dataThread implements Runnable {
		...
		public void run() {
			...
			String str = update_history_pc_dataServer(userId,
					...
					historyBPMTime[countBPMResult], 0, screenName_,
					screenName_, 0);
	...
	private String update_history_pc_dataServer(int userId,
			int systolicPressure, int diastolicPressure, int pulse, int oxygen,
			String uploadTime, int uploadType, String userName,
			String screenName, int familyMember) {
		String str = "";
		try {
			// URL url = new
			// URL("http://192.168.136.9:8080/ZKYweb/Upload_PC_DataServlet");
			// URL url = new
			// URL("http://localhost:8080/ZKYweb/Upload_PC_DataServlet");
			// URL url = new
			// URL("http://192.168.52.103:8080/ZKYweb/Upload_PC_DataServlet");
	...
	class Update_pc_dataThread implements Runnable {
		...
		public void run() {
			...
			String str = update_pc_dataServer(userId,
			...
			if (str.equals("success")) {
				...
				System.out.println(str+"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
				msg.what = 1;
				handler.sendMessage(msg);
			}
	...
	class Update_pc_bgdataThread implements Runnable {
		...
		public void run() {
			...
			String str = update_pc_bgdataServer(userId, ThisGLU, uploadTime,
	...
	class Update_history_pc_bgdataThread implements Runnable {
		...
		public void run() {
			...
			String str = update_history_pc_bgdataServer(userId,
					...
					historyGLUTime[countBPMResult], screenName_, screenName_, 0);
	...
	private String update_history_pc_bgdataServer(int userId,
			String bloodGlucose, String uploadTime, String userName,
			String screenName, int familyMember) {
		String str = "";
		try {
